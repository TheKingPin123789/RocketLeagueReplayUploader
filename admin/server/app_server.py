...
def make_token(machine_guid: str) -> str:
    return hmac.new(get_secret(), machine_guid.encode(), hashlib.sha256).hexdigest()  # type: ignore

def verify_token(machine_guid: str, token: str) -> bool:
    return hmac.compare_digest(make_token(machine_guid), token)
...
def discord_ping(title: str, fields: list, color: int = 3447003):
    try:
        req.post(WEBHOOK, json={"embeds": [{"title": title, "color": color, "fields": fields}]}, timeout=5)
    except Exception:
        pass
...
<div id="panel-users" class="panel active">
  <div class="section">
    <table>
      <thead><tr><th>Username</th><th>GUID</th><th>Tier</th><th>Expiry</th><th>Registered</th><th>Last IP</th><th>Last Seen</th><th>Actions</th></tr></thead>
...
def register():
    ...
    guid     = data.get("machine_guid", "").strip()
    ...
    if guid in users:
        token = users[guid]["token"]
        return jsonify({"token": token, "tier": users[guid]["tier"], "status": "existing"})

    token = make_token(guid)
    ...
    save_users(users)
    log("register", "new user", ip=ip, user=username)
    discord_ping("New Registration", [
        {"name": "User",  "value": username,      "inline": True},
        {"name": "GUID",  "value": guid,           "inline": True},
        {"name": "IP",    "value": ip,             "inline": True},
        {"name": "Tier",  "value": "free_tester",  "inline": True},
    ], color=5763719)
...
def verify():
    ...
    if guid not in users:
        ...
        discord_ping("Unregistered Access Attempt", [
            {"name": "GUID", "value": guid, "inline": True},
            {"name": "IP",   "value": ip,   "inline": True},
        ], color=15548997)
    ...
    if not verify_token(guid, token):
        log("verify", "invalid token", ip=ip, user=user.get("username", "?"), level="warn")
        discord_ping("Invalid Token Attempt", [
            {"name": "User", "value": user.get("username", "?"), "inline": True},
            {"name": "GUID", "value": guid, "inline": True},
            {"name": "IP",   "value": ip,   "inline": True},
        ], color=15548997)
    ...
    if tier == "revoked":
        log("verify", "revoked", ip=ip, user=user.get("username", "?"), level="warn")
        discord_ping("Revoked User Access Attempt", [
            {"name": "User", "value": user.get("username", "?"), "inline": True},
            {"name": "GUID", "value": guid, "inline": True},
        ], color=15548997)
    ...
    if expiry:
        ...
        if time.time() > expiry_ts:
            log("verify", "expired", ip=ip, user=user.get("username", "?"), level="warn")
            return jsonify({"status": "expired", "tier": tier}), 403

    users[guid]["last_seen"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
    users[guid]["ip"] = ip
    save_users(users)
    log("verify", f"ok · {tier}", ip=ip, user=user.get("username", "?"))
...
def serve_code():
    ...
    if users[guid].get("tier") == "revoked":
        return jsonify({"error": "revoked"}), 403
    if not CODE_FILE.exists():
        return jsonify({"error": "code not available"}), 503

    log("code", "served", ip=request.remote_addr, user=users[guid].get("username", "?"))
...
def admin_login():
    ...
    if not check_admin_proof(guid, proof):
        log("admin", "wrong password", ip=request.remote_addr, level="warn")
...
def dash_push_code(token):
    ...
    discord_ping("Code Updated", [
        {"name": "Version", "value": version or get_version(), "inline": True},
    ], color=5763719)
...
def revoke_user():
    ...
    save_users(users)
    discord_ping("User Revoked", [
        {"name": "User", "value": users[guid].get("username", "?"), "inline": True},
        {"name": "GUID", "value": guid, "inline": True},
    ], color=15548997)
